...
windowSurface = pygame.display.set_mode((600, 600), 0, 32)  # 600x600 px
...
class Labyrinth:
    ...
    def mac_move(self, event):
        if event.key == pygame.K_LEFT:  # if left arrow
            if maze[self.macgyver.x // 40-1][self.macgyver.y // 40] == " " or\
                    maze[self.macgyver.x //
                         40 - 1][self.macgyver.y // 40] == "m":
            ...
            if maze[self.macgyver.x // 40][self.macgyver.y // 40+1] == " " or \
                    maze[self.macgyver.x //
                         40][self.macgyver.y // 40 + 1] == "v":
                self.macgyver.y = self.macgyver.y + 40  # move down
            else:
                pygame.mixer.music.play()  # play collision sound

    def get_item(self):
        if self.macgyver.x == self.pos_needle.x and \
                self.macgyver.y == self.pos_needle.y:
            # if MacGyver reaches needle
            self.pos_needle.picked = True
            self.macgyver.loot[0] = True
        if self.macgyver.x == self.pos_tube.x and \
                self.macgyver.y == self.pos_tube.y:
            # if MacGyver reaches tube
            self.pos_tube.picked = True
            self.macgyver.loot[1] = True
        if self.macgyver.x == self.pos_ether.x and \
                self.macgyver.y == self.pos_ether.y:
            # if MacGyver reaches ether
            self.pos_ether.picked = True
            self.macgyver.loot[2] = True

    def end_game(self, event):
        if self.macgyver.x == self.murdock.x and \
                self.macgyver.y == self.murdock.y:
            # if MacGyver reaches Murdock
            self.end = True
            if self.macgyver.loot[0] and \
                    self.macgyver.loot[1] and self.macgyver.loot[2]:
                self.win = True
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_c:
                        # replay the game, reload initial conditions
    ...
    def display(self):
        ...
        windowSurface.blit(self.sprite.img_macgyver,
                           (self.macgyver.x, self.macgyver.y))
        # display MacGyver
        if self.pos_needle.picked is False:
            windowSurface.blit(self.sprite.img_needle,
                               (self.pos_needle.x, self.pos_needle.y))
            # display needle on the labyrinth
        else:
            windowSurface.blit(self.sprite.inv_needle, (0, 0))
            # display needle in the inventory
        if self.pos_tube.picked is False:
            windowSurface.blit(self.sprite.img_tube,
                               (self.pos_tube.x, self.pos_tube.y))
            # display tube on the labyrinth
        else:
            windowSurface.blit(self.sprite.inv_tube, (0, 40))
            # display tube in the inventory
        if self.pos_ether.picked is False:
            windowSurface.blit(self.sprite.img_ether,
                               (self.pos_ether.x, self.pos_ether.y))
            # display ether in the labyrinth
        else:
            windowSurface.blit(self.sprite.inv_ether, (0, 80))
            # display ether in the inventory
        if self.end and self.win:
            windowSurface.blit(self.sprite.screen_win, (0, 0))
            # display victory screen
        if self.end and self.win is False:
            windowSurface.blit(self.sprite.screen_lose, (0, 0))
